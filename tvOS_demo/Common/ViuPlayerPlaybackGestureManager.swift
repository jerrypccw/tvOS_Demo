...
class ViuPlayerPlaybackGestureManager: NSObject, UIGestureRecognizerDelegate {
    weak open var delegate: ViuPlayerPlaybackGestureManagerDelegate? // the gesture recognizer's delegate
    var touchLocation: TouchRemoteLocation = .center
    
    func addGesture(_ target: UIView) {
        let touch = ViuPlayerPlaybackTouchGestureRecognizer(target: self, action: #selector(touchGesture(_:)))
        touch.delegate = self
    ...
    @objc func touchGesture(_ gesture: ViuPlayerPlaybackTouchGestureRecognizer) {
...
public protocol ViuPlayerPlaybackGestureManagerDelegate : NSObjectProtocol {
