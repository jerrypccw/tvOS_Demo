//
//  ViuPlayerCacheMediaWorker.swift
//  tvOS_demo
//
...
open class ViuPlayerCacheMediaWorker: NSObject {
    open fileprivate(set) var cacheConfiguration: ViuPlayerCacheMediaConfiguration?
    ...
    fileprivate let writeFileQueue = DispatchQueue(label: "com.Viuplayer.cacheWriteFileQueue")
    ...
    public init(url: URL) {
        let path = ViuPlayerCacheManager.cacheFilePath(for: url)
        filePath = path
        ...
        if err == nil {
            ...
            if err == nil {
                ...
                do {
                    ...
                    cacheConfiguration = ViuPlayerCacheMediaConfiguration.configuration(filePath: path)
    ...
    open func cachedDataActions(forRange range:NSRange) -> Array<ViuPlayerCacheAction> {
        var actions = [ViuPlayerCacheAction]()
            ...
            for (_, value) in cachedSegments.enumerated() {
                ...
                if intersctionRange.length > 0 {
                    ...
                    for i in 0...package {
                        ...
                        let action = ViuPlayerCacheAction(type: .local, range: ra)
        ...
        if actions.count == 0 {
            let action = ViuPlayerCacheAction(type: .remote, range: range)
            actions.append(action)
        } else {
            var localRemoteActions = [ViuPlayerCacheAction]()
            for (index, value) in actions.enumerated() {
                ...
                if index == 0 {
                    if range.location < actionRange.location {
                        ...
                        let action = ViuPlayerCacheAction(type: .remote, range: ra)
    ...
    open func set(cacheMedia: ViuPlayerCacheMedia) -> Bool {
