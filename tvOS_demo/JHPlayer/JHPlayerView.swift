...
//    open var volumeSlider : UISlider!
    ...
    open fileprivate(set) var panGestureDirection : JHPlayerViewPanGestureDirection = .horizontal
...
//        singleTapGesture.require(toFail: doubleTapGesture)
        ...
        let swipeRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(swiped(_:)) )
...
//    @objc internal func timeSliderValueChanged(_ sender: JHPlayerSlider) {
//        isTimeSliding = true
//        if let duration = jhPlayer?.totalDuration {
...
//    }
//
//    @objc internal func timeSliderTouchDown(_ sender: JHPlayerSlider) {
//        isTimeSliding = true
//        timer.invalidate()
//    }
//
//    @objc internal func timeSliderTouchUpInside(_ sender: JHPlayerSlider) {
//        isTimeSliding = true
//
//        if let duration = jhPlayer?.totalDuration {
//            let currentTime = Double(sender.value) * duration
//            jhPlayer?.seekTime(currentTime, completion: { [weak self] (finished) in
//                guard let strongSelf = self else { return }
//                if finished {
    ...
    @objc open func onPanGesture(_ gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            case .horizontal:
                ...
                self.jhPlayer?.seekTime(sliderSeekTimeValue, completion: { [weak self] (finished) in
                    guard let strongSelf = self else { return }
                    if finished {
                        strongSelf.isTimeSliding = false
                        strongSelf.setupTimer()
...
//        isVolume ? (volumeSlider.value -= Float(velocityY / 10000)) : (UIScreen.main.brightness -= velocityY / 10000)
    ...
    @objc internal func onCloseView(_ sender: UIButton) {
        delegate?.jhPlayerView(didTappedClose: self)
    }
    
    @objc internal func onReplay(_ sender: UIButton) {
        jhPlayer?.replaceVideo((jhPlayer?.contentURL)!)
        jhPlayer?.play()
    }
...
//        delegate?.jhPlayerView(self, willFullscreen: isFullScreen)
//    }
}
